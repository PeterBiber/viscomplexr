...
# Variante 1 - ganz alte Codierung
...
# Variante 3 - Funktion außerhalb definieren
x11(width = 8, height = 8)
...
n <- 200
a <- complex(n, modulus = runif(n), argument = 2*pi*runif(n))
aString <- vector2String(a)

# Variante 1 - ganz alte Codierung
x11(width = 8, height = 8)
system.time(
    phasePortrait(paste("vapply(z, function(z, a){
                         return(prod(abs(a)/a * (a-z)/(1-Conj(a)*z)))
                         },", aString, ", FUN.VALUE = complex(1))"),
                  pType = "p",
                  xlim = c(-1.7, 1.7), ylim = c(-1.7, 1.7), tempDir = tempDir,
                  xlab = "real", ylab = "imaginary")
)
