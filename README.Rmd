...
Let's plot a phase portrait of the function `f(z)=sin(z^3/(z-1i+2))`. This code produces exactly the banner shown above. The package's workhorse function is called `phasePortrait`, and besides its specific input variables, it also accepts all parameters that are accepted by the `plot.default` function, like `axes`, `xaxs`, and `yaxs` in the example below. Other often useful ones are `main`, `xlab`, and `ylab`, especially for scientific, technical or educational applications.
