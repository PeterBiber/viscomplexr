...
realm of complex-valued functions. The package works in the framework of
R base graphics and allows for print quality graphics files. To a great
deal, the package follows the conventions published by Wegert (2012).
...
This code produces exactly the banner shown above. The package’s
workhorse function is called `phasePortrait`, and besides its specific
input variables, it also accepts all parameters that are accepted by the
`plot.default` function, like `axes`, `xaxs`, and `yaxs` in the example
below. Other often useful ones are `main`, `xlab`, and `ylab`,
especially for scientific, technical or educational applications.
...
phase portraits. The example could even be reduced to just the function
to be portrayed (here provided as a character string) and the parameters
