---
title: "Phase Portraits of Complex Valued Functions - the R Package viscomplexr"
author: "Peter Biber"
...
```{r figure_3, fig.width=7, fig.height=7, results='hide', fig.align='center', cache=TRUE, fig.show='hold', fig.cap='Phase portrait of the function $f(z)=\\frac{(3+2\\mathrm{i}+z)(-5+5\\mathrm{i}+z)}{(-2-2\\mathrm{i}+z)^2}$ in the window $\\left|\\Re(z)\\right| < 8.5$ and $\\left|\\Im(z)\\right| < 8.5$.'}
...
```{r figure_7, fig.width = 5, fig.height = 5, results = 'hide', fig.align='center', fig.show='hold', cache = TRUE, fig.cap = 'Phase portrait of the function $f(z)=\\mathrm{e}^z$ in the window $\\left|\\Re(z)\\right| < 8.5$ and $\\left|\\Im(z)\\right| < 8.5$ with iso-modulus lines.'}
op <- par(mar = c(5.1, 4.1, 2.1, 2.1), cex = 0.8) # adjust plot margins 
                                                  # and general text size
...
```{r figure_9, fig.width=7, fig.height=7, fig.show='hold', results='hide', fig.align='center', cache=TRUE, fig.cap='Tuning reference zone contrast with the parameters `darkestShade` (column-wise, 0, 0.2, 0.4), and `lambda` (row-wise, 0.1, 1, 10).'}
...
```{r, figure_14, fig.width=7, fig.height=2/3*7, fig.align='center', results='hide', fig.show='hold', cache=TRUE}
...
```{r, eval=FALSE, fig.width=7, fig.height=2/3*7, fig.align='center', results='hide', fig.show='hold', cache=TRUE}
phasePortrait("tan(z^3 + 1/2 - 2i)/(1 - 1i - z)", 
              xlim = c(-6, 6), ylim = c(-3, 3),
              axes = FALSE)
```

Note the unequal figure margins. These are **R**-default in order to reserve space for the usual annotations. Adjusting them is possible by means of the graphics parameters `mar` or `mai` as explained above.

<!-- Load the plot as an image, 150 dpi -->
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("figure_axes_left_away.png")
```
...
<!-- Load the plot as an image, 150 dpi -->
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("figure_with_box.png")
```
...
<!-- Load the plot as an image, 150 dpi -->
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("figure_black_bg_white_text.png")
```
...
<!-- Load the plot as an image, 150 dpi -->
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("figure_full_cover_device.png")
```
...
<!-- Load the plot as an image, 150 dpi -->
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("figure_equal_margins.png")
```
