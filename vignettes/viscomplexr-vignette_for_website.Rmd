---
title: "Phase Portraits of Complex Functions - the R Package viscomplexr"
author: "Peter Biber"
output: rmarkdown::html_vignette
header-includes: \usepackage{amsmath}
bibliography: REFERENCES.bib
vignette: |
  %\VignetteIndexEntry{viscomplexr-vignette_for_website} 
...
While the text of this article is virtually identical with the package's vignette as available on [CRAN](https://CRAN.R-project.org/package=viscomplexr), CRAN's size restrictions do not apply here. Therefore, you will find considerably more illustrations below.
...
```{r, figure_1, fig.width = 8, fig.height = 8, results = 'hide', fig.align='center', cache = FALSE, fig.show = 'hold', fig.cap = 'Phase portrait of the function $f(z)=z$ in the window $\\left|\\Re(z)\\right| < 8.5$ and $\\left|\\Im(z)\\right| < 8.5$.'}
...
```{r figure_2, fig.width=8, fig.height=8, results="hide", fig.align='center', fig.show='hold', cache=TRUE, fig.cap= "Different options for including reference lines with the argument `pType`."}
...
```{r figure_3, fig.width=8, fig.height=8, results='hide', fig.align='center', cache=TRUE, fig.show='hold', fig.cap='Phase portrait of the function $f(z)=\\frac{(3+2\\mathrm{i}+z)(-5+5\\mathrm{i}+z)}{(-2-2\\mathrm{i}+z)^2}$ in the window $\\left|\\Re(z)\\right| < 8.5$ and $\\left|\\Im(z)\\right| < 8.5$.'}
...
```{r figure_5, fig.width=8, fig.height=3.2, results='hide', fig.align='center', , fig.show='hold', cache=TRUE, fig.cap='The function $f(z)=\\frac{(3+2\\mathrm{i}+z)(-5+5\\mathrm{i}+z)}{(-2-2\\mathrm{i}+z)^2}$ portrayed with three different settings of `pi2Div` and `pType = "pma"`.'}
# divide graphics device into three regions and adjust plot margins 
op <- par(mfrow = c(1, 3), mar = c(0.2, 0.2, 0.4, 0.2))
for(n in c(6, 9, 18)) {
  phasePortrait("(3+2i+z)*(-5+5i+z)/(-2-2i+z)^2", xlim = c(-8.5, 8.5), ylim = c(-8.5, 8.5),
...
```{r figure_6, fig.width=8, fig.height=3.2, results='hide', fig.align='center', , fig.show='hold', cache=TRUE, fig.cap='The function $f(z)=\\frac{(3+2\\mathrm{i}+z)(-5+5\\mathrm{i}+z)}{(-2-2\\mathrm{i}+z)^2}$ portrayed with decoupled settings of `pi2Div` and `logBase`.'}
# divide graphics device into three regions and adjust plot margins 
op <- par(mfrow = c(1, 3), mar = c(0.2, 0.2, 0.4, 0.2))
for(n in c(6, 9, 18)) {
  phasePortrait("(3+2i+z)*(-5+5i+z)/(-2-2i+z)^2", xlim = c(-8.5, 8.5), ylim = c(-8.5, 8.5),
...
```{r figure_7, fig.width = 8, fig.height = 8, results = 'hide', fig.align='center', fig.show='hold', cache = TRUE, fig.cap = 'Phase portrait of the function $f(z)=\\mathrm{e}^z$ in the window $\\left|\\Re(z)\\right| < 8.5$ and $\\left|\\Im(z)\\right| < 8.5$ with iso-modulus lines.'}
op <- par(mar = c(5.1, 4.1, 2.1, 2.1), cex = 0.8) # adjust plot margins 
                                                  # and general text size
...
```{r figure_8, fig.width=8, fig.height=3.2, results='hide', fig.align='center', fig.show='hold', cache=TRUE, fig.cap='The function $f(z)=\\mathrm{e}^z$ portrayed with the default coupling of `pi2Div` and `logBase` as implemented in *phasePortrait*.'}
# divide graphics device into three regions and adjust plot margins 
op <- par(mfrow = c(1, 3), mar = c(0.2, 0.2, 0.4, 0.2))
for(n in c(6, 9, 18)) {
...
```{r figure_9, fig.width=8, fig.height=8, fig.show='hold', results='hide', fig.align='center', cache=TRUE, fig.cap='Tuning reference zone contrast with the parameters `darkestShade` (column-wise, 0, 0.2, 0.4), and `lambda` (row-wise, 0.1, 1, 10).'}
...
```{r figure_10, fig.width=8, fig.height=3.2, results='hide', fig.align='center', fig.show='hold', cache=TRUE, fig.cap='Three phase portraits with branch cuts (dashed line), illustrating the three values of $f(z)=z^{1/3}$, $z \\in \\mathbb{C} \\setminus \\lbrace 0 \\rbrace$. The transitions between the phase portraits are indicated by same-coloured arrows pointing at the branch cuts.'}
...
```{r figure_11, fig.width=8, fig.height=3.2, fig.align='center', results='hide', fig.show='hold', cache=TRUE, fig.cap='Three branches of $\\log z=\\log r+\\mathrm{i}\\cdot(\\varphi + k\\cdot2\\pi), r>0, \\varphi\\in\\left[0,2\\pi\\right[$, with $k=-1,0,1$. The branch cuts are marked with dashed white lines.'}
op <- par(mfrow = c(1, 3), mar = c(0.4, 0.2, 0.2, 0.2))
for(k in -1:1) {
...
```{r figure_12, fig.width=8, fig.height=4, fig.align='center', results='hide', fig.show='hold', cache=TRUE, fig.cap='Mapping the complex number plane on the Riemann sphere. Left: lower (southern) hemisphere; right upper (northern hemisphere). Folding both figures face to face along a vertical line in the middle between them can be imagined as closing the Riemann sphere.'}
...
```{r figure_13, fig.width=8, fig.height=4.2, fig.align='center', results='hide', fig.show='hold', cache=TRUE, fig.cap='Riemann sphere plot of the function $f(z)=\\frac{(z^{2}+\\frac{1}{\\sqrt{2}}+\\frac{\\mathrm{i}}{\\sqrt{2}})\\cdot(z+\\frac{1}{2}+\\frac{\\mathrm{i}}{2})}{z-1}$. Annotated are the zeroes $z_1$, $z_2$, $z_3$, and the poles $z_4$, $z_5$.'}
...
```{r, figure_14, fig.width=8, fig.height=2/3*8, fig.align='center', results='hide', fig.show='hold', cache=TRUE}
...
<!-- Load the plot as an image, 150 dpi only -->
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("figure_Mandelbrot_Example.png")
```
...
<!-- Load the plot as an image, 150 dpi only -->
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("figure_Julia_Example_1.png")
```
...
<!-- Load the plot as an image, 150 dpi only -->
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("figure_Julia_Example_2.png")
```
...
```{r, figure_15, fig.width=8, fig.height=8, fig.align='center', results='hide', fig.show='hold', cache=TRUE, fig.cap="Two color portraits of the function $f(z)=z$ made with `phasePortraitBw` and `byType` 'ma' (top left), 'a' (top right), and 'm' (bottom left) together with a full phasePortrait (bottom right)."}
...
```{r figure_16, fig.width=8, fig.height=8, fig.align='center', results='hide', fig.show='hold', cache=TRUE, fig.cap="Two color portraits of the function $f(z)=\\frac{(z+\\sqrt{2}\\cdot\\mathrm{i}-\\sqrt{2})^2}{z^3 + 2}$ made with `phasePortraitBw` and `byType` 'ma' (top left), 'a' (top right), and 'm' (bottom left) together with a full phasePortrait (bottom right)."}
...
### Dealing with axes
Usually, when aiming for mainly aesthetic effects, you want to suppress plot axes from being drawn. As phase *phasePortrait* and *phasePortraitBw* accept, via the `...` argument, all arguments also accepted by **R**'s `plot.default`,
this can be easily achieved by providing the argument `axes = FALSE`:

```{r, figure_17, fig.width=8, fig.height=2/3*8, fig.align='center', results='hide', fig.show='hold', cache=TRUE, fig.cap="Phase portrait called with `axes = FALSE`. Note the unequal figure margins. These are **R**-default in order to reserve space for the usual annotations. Adjusting them is possible by means of the graphics parameters `mar` or `mai` as explained above."}
...
```{r, figure_18, fig.width=8, fig.height=2/3*8, fig.align='center', results='hide', fig.show='hold', cache=TRUE}
phasePortrait("tan(z^3 + 1/2 - 2i)/(1 - 1i - z)", 
              xlim = c(-6, 6), ylim = c(-3, 3),
              axes = FALSE)
box()
...
```{r, figure_19, fig.width=8, fig.height=2/3*8, fig.align='center', results='hide', fig.show='hold', dev.args=list(bg = 'black', fg = 'white'), cache=TRUE}
...
```{r, figure_20, fig.width=8, fig.height=9/16*8, fig.align='center', results='hide', fig.show='hold', cache=TRUE}
...
```{r, figure_21, fig.width=8, fig.height=9/16*8, fig.align='center', results='hide', fig.show='hold', cache=TRUE, dev.args=list(bg='black')}
...
### Ways to provide functions to *phasePortrait* and *phasePortraitBw* {#ways_functions}
#### Quoted character strings
Any function to be visualized with *phasePortrait* or *phasePortraitBw* must be provided as the argument `FUN`. In some cases (see below), the argument `moreArgs` can turn out useful in combination with `FUN`. Probably the easiest way of defining `FUN` is a character string which is an expression **R** can evaluate as a function of a complex number $z$. See some examples:
