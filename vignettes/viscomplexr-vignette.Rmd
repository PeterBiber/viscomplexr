...
Probably, the most useful application of this concept is when the `vapply` construct is pasted together at runtime with values for the additional arguments depending on what happened earlier.




+++ Bruchstück: Wenn vordefinierte Funktionen +++
It is better to use *phasePortrait's* argument `moreArgs` instead. `moreArgs` must be a named list (names must match the names of the required arguments), where the argument values are assigned:


#### Function objects
+++ Prüfe: kann man auch direkt im Aufruf Funktion definieren? +++
